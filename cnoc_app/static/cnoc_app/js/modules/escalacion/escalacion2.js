...
import { formatDateValue, normalizeDateForBackend  } from '../../helpers/dates.js';
// # LAS APIS 
...
/* -  SENCILLO, DESPUES CAMBIAR AL HELPER DE DOM   - */ 
    ...
    async function buscarDatos_api() {
        ...
        try {
            ...
            if (currentTimeInput && row.OPEN_TIME) {
                currentTimeInput.value = formatDateValue(row.OPEN_TIME, 'datetime-local');
            }
...
async function recalcularTablaEscalacion() {
    const areaId = document.getElementById('areasxpais')?.value || '';
    const nivel = document.getElementById('acumulado')?.value || '1';    
    const hrActualOverride = ( document.getElementById('current_time')?.value || ''  ); // normalizeDateForBackend
    const tk = (document.getElementById('falla')?.value || '').trim();

    if (!tk) {
        console.warn('⚠️ No hay falla cargada para recalcular');
        return;
    }
    if (!hrActualOverride) {
        console.warn('⚠️ No hay HORA para recalcular');
        return;
    }
    if (!areaId) {
        console.warn('⚠️ No se seleccionó área');
